![image](../../img/terminology/AESIdentity.png)
...
This ID is used for `Focus Event` registrations where you can register to listen for events from an object that has a **specific ID**. In order to identify which object we want to listen to events from, we have to know their `Object ID` - hence why this field is exposed. It allows you to override the default behavior of how this Object ID is generated and assign your own unique naming to individual objects should you choose to do so.

The `Target Id` can also be fetched from the `Identity`.

?>**NOTE**: If this is run on the `Server` as well as the `Client`, the unique ID will differ do to each side of the wire generating it's own unique GUID. <br><br>See ["FAQ - Is this replicated" for more details.](../faq.md)
...
This identifies the type of object by using a single `Gameplay Tag`. This gameplay tag is used to allow objects to register for `Type Events` to allow the object to listen for events from a specific `type` of object. For example a player can register for events that come from any object that has a type of `Animal.Chicken`.
...
?>**NOTE**: This assumes that when the Chicken generated its `Identity`, the `Object Type` that was passed in was `Animal.Chicken`. This also assumes the Dragons' `Object Type` is not an `Animals.Chicken`😏
